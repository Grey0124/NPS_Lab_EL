...
class ARPSpoofingDetector:
    ...
    def load_ml_model(self):
        ...
        if not self.model_path or not os.path.exists(self.model_path):
            self.logger.warning("No ML model found, using rule-based detection only")
            return
    ...
    def process_packet(self, packet):
        ...
        try:
            ...
            if is_spoofing or (ml_prediction and ml_prediction != 0 and ml_confidence > self.detection_threshold):
                detection_result['combined_threat'] = True
                detection_result['threat_level'] = 'HIGH' if ml_confidence > 0.8 else 'MEDIUM'
